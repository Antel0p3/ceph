...
int ECUtil::decode(
  ...
  // Determine n_stripes from the first available data shard; fall back to
  // parity shards (using their per-shard chunk size) when no data shard present.
...
int ECUtil::decode(
  ...
  // Derive chunks_count from the first DATA shard in min; fall back to parity.
  int chunks_count = 0;
  for (auto& [id, bl] : to_decode) {
