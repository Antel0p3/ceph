  ...
  const ZTracer::Trace &trace)
{
  ...
  if (rop.do_redundant_reads || rop.in_progress.empty()) {
	...
	// If attrs were requested but not received (the shard that should have
	// provided them returned an error), retry from another available shard.
	if (rop.complete[iter->first].r == 0 &&
	    rop.to_read.at(iter->first).want_attrs &&
	    (!rop.complete[iter->first].attrs ||
	     rop.complete[iter->first].attrs->empty())) {
	  int r = send_all_remaining_reads(iter->first, rop);
	  if (r == 0) {
	    need_resend = true;
	    continue;
	  }
	  dout(10) << __func__ << " unable to get attrs for " << iter->first
		   << " from any remaining shard, failing recovery" << dendl;
	  rop.complete[iter->first].r = -EIO;
	}
