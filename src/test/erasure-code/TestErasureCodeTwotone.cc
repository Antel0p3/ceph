...
TYPED_TEST(ErasureCodeTest, encode)
{
  TypeParam twotone;
  ErasureCodeProfile profile;
  profile["k"] = "3";
...
TYPED_TEST(ErasureCodeTest, encode_decode)
{
  ...
  for (int per_chunk_alignment = 0 ;
    ...
    profile["m"] = "2";
    profile["packetsize"] = "8";
    profile["twotone-per-chunk-alignment"] =
...
#define LARGE_ENOUGH 2048
    ...
    map<int, bufferlist> encoded;
    EXPECT_EQ(0, twotone.encode(set<int>(want_to_encode, want_to_encode+4),
				 in,
				 &encoded));
    printf("0: %u %s\n\n", encoded[0].length(), encoded[0].c_str());
    printf("1: %u %s\n\n", encoded[1].length(), encoded[1].c_str());
    // printf("1: %s\n\n", encoded[1].c_str());
    printf("in: %s\n", in.c_str());
    // printf("2: %s\n", encoded[2].c_str());
    // printf("3: %s\n", encoded[3].c_str());
    ...
    // all chunks are available
    {
      ...
      map<int, bufferlist> decoded;
      EXPECT_EQ(0, twotone._decode(set<int>(want_to_decode, want_to_decode+2),
				    encoded,
				    &decoded));
...
TYPED_TEST(ErasureCodeTest, minimum_to_decode)
{
  ...
  //
  {
    ...
    EXPECT_EQ(0, twotone._minimum_to_decode(want_to_read,
					     available_chunks,
					     &minimum));
  ...
  //
  {
    ...
    EXPECT_EQ(-EIO, twotone._minimum_to_decode(want_to_read,
						available_chunks,
						&minimum));
  }
  //
...
{
  ErasureCodeTwotoneReedSolomonVandermonde twotone;
  ErasureCodeProfile profile;
  profile["k"] = "2";
  profile["m"] = "2";
  profile["w"] = "8";
  ...
  unsigned aligned_object_size = twotone.get_alignment() * 2;
  {
    //
...

  {
    ...
    // contains the first chunk. Although the twotone encode
...
TEST(ErasureCodeTest, create_rule)
{
...

  {
    ...
    profile["w"] = "8";
    twotone.init(profile, &cerr);
    int rule = twotone.create_rule("myrule", *c, &ss);
    EXPECT_EQ(0, rule);
    EXPECT_EQ(-EEXIST, twotone.create_rule("myrule", *c, &ss));
    //
    ...
    vector<int> out;
    int x = 0;
    c->do_rule(rule, x, out, twotone.get_chunk_count(), weight, 0);
    ASSERT_EQ(out.size(), twotone.get_chunk_count());
  ...
  }
  {
    ...
    twotone.init(profile, &cerr);
    EXPECT_EQ(-ENOENT, twotone.create_rule("otherrule", *c, &ss));
  ...
  }
  {
    ...
    twotone.init(profile, &cerr);
    EXPECT_EQ(-EINVAL, twotone.create_rule("otherrule", *c, &ss));
 ...
 * Local Variables:
 * compile-command: "cd ../.. ;
 *   make -j4 unittest_erasure_code_twotone &&
 *   valgrind --tool=memcheck \
 *      ./unittest_erasure_code_twotone \
