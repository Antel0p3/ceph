...
void ErasureCodeTwotoneImpl::twotone_encode(char **data, char **coding, int blocksize)
{
  ...
  // Init each parity from the data chunk with shift=0 (avoids full memset pass).
  // twotone_shift() guarantees that for every parity p there exists at least
  // one data chunk d* where shift[p][d*] = 0.
  int init_d[m];
  for (int p = 0; p < m; ++p) {
    init_d[p] = -1;
    for (int d = 0; d < k; ++d) {
  ...
  // Fused tile XOR — load data[d] once, XOR all active parities.
  for (int d = 0; d < k; ++d) {
    char* pp[m];
    int npp = 0;
    for (int p = 0; p < m; ++p) {
                                ...
                                const bool *missing_chunk)
{
    for (int p = 0; p < m; ++p) {
        ...
        const size_t parity_bytes __attribute__((unused)) =
            chunk_bytes + layout.max_shift[p] * BYTE_PERCELL;
        char* parity = P[p];
...
int ErasureCodeTwotoneImpl::twotone_decode(int *erasures, char **data, char **coding, int blocksize)
{
    ...
    if (missing_data_count == 1) {
        ...
        // Prefer the parity with the smallest max_shift.
        int p_use = -1;
        for (int p = 0; p < m; ++p) {
